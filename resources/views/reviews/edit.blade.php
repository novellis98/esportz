<x-layout title="modifica recensione">
    <div class="container my-5">
        <div class="row justify-content-center">
            <div class="col-12 col-md-8 col-lg-6 d-flex flex-column gap-4">
                <h3 class="text-center">Modifica la recensione</h3>
                <!-- Form -->
                <div class="form-container">
                    <form action="{{ route('reviews.update', $review) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <input type="hidden" name="article_id" value="{{ $review->article->id }}">
                        <div class="mb-3 d-flex justify-content-center gap-2" style="font-size: 1.5rem">
                            @for ($i = 1; $i <= 5; $i++)
                                ...
                                <label for="star{{ $i }}" class="star">
                                    <i
                                        class="bi {{ $review->rating >= $i ? 'bi-star-fill' : 'bi-star' }} text-warning star-icon"></i>
                                </label>
                            @endfor
                        </div>
                        <script>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" id="body" name="body" rows="3">{{ $review->body }}</textarea>
                        </div>
                        <button type="submit" class="btn btn-custom mt-3">Modifica recensione</button>
                    </form>
                </div>
